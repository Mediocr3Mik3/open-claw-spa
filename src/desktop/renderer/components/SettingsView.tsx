...
export default function SettingsView({ gwOn, brOn, gwUrl, setGwUrl, configKeys, setConfigKeys }: {
  ...
  return (
    <div style={{ padding: 28, overflowY: "auto" as const, flex: 1, animation: "fadeIn .2s ease" }}>
      ...
      <p style={{ fontSize: 13, color: C.dim, marginBottom: 22 }}>Configuration, providers, adapters, and organization.</p>
      ...
      {sub === "general" && (<>
        ...
        {logs.length > 0 && (
          <div style={{ ...glass(0), padding: 10, maxHeight: 140, overflowY: "auto" as const, fontFamily: "'SF Mono', monospace", fontSize: 11, marginBottom: 22 }}>
        ...
        <div style={{ ...glass(0), overflow: "hidden" }}>
          ...
          {configKeys.map(k => (
            <div key={k} style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "8px 12px", borderBottom: `1px solid ${C.border}` }}>
              <span style={{ fontFamily: "'SF Mono', monospace", fontSize: 11 }}>{k}</span>
      ...
      {sub === "llm" && (<>
        ...
        <p style={{ fontSize: 12, color: C.dim, marginBottom: 14 }}>Encrypted with AES-256-GCM, backed by OS keychain.</p>
        ...
        <div style={{ ...glass(0), overflow: "hidden", marginBottom: 22 }}>
          ...
          {vaultKeys.map(k => (
            <div key={k} style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "9px 14px", borderBottom: `1px solid ${C.border}` }}>
              <div style={{ display: "flex", alignItems: "center", gap: 8 }}><Dot color={C.ok} /><span style={{ fontFamily: "'SF Mono', monospace", fontSize: 12 }}>{k}</span></div>
        ...
        <div style={{ display: "flex", gap: 8, marginBottom: 14 }}>
          <div style={{ flex: 1 }}>
            <label style={{ fontSize: 10, color: C.dim, display: "block", marginBottom: 4 }}>Monthly Limit (USD)</label>
            <Input value={budgetLimit} onChange={setBudgetLimit} type="number" />
          </div>
          <div style={{ flex: 1 }}>
            <label style={{ fontSize: 10, color: C.dim, display: "block", marginBottom: 4 }}>Warn at (%)</label>
            <Input value={budgetWarn} onChange={setBudgetWarn} type="number" />
          </div>
        ...
        <div style={{ display: "flex", gap: 8, marginBottom: 14 }}>
          ...
          <div style={{ display: "flex", gap: 2, background: C.surface, borderRadius: 8, padding: 3, border: `1px solid ${C.border}` }}>
            {(["all", "local", "api"] as const).map(f => (
              <button key={f} onClick={() => setModelFilter(f)} style={{ padding: "6px 12px", borderRadius: 5, border: "none", background: modelFilter === f ? C.accentSoft : "transparent", color: modelFilter === f ? C.accent : C.dim, fontSize: 11, fontWeight: 600, textTransform: "uppercase" as const }}>{f}</button>
            ))}
          </div>
        </div>
        ...
        {runtimes.length === 0 ? (
          <Card style={{ marginBottom: 14 }}><div style={{ display: "flex", alignItems: "center", gap: 8 }}><Dot color={C.warn} /><span style={{ fontSize: 13 }}>No local runtimes detected.</span></div></Card>
        ) : runtimes.map((r: any) => (
      ...
      {sub === "adapters" && (<>
        ...
        <p style={{ fontSize: 12, color: C.dim, marginBottom: 14 }}>Configure tokens in Encrypted Config (General tab). Bridge auto-enables adapters with tokens.</p>
          ...
          <div style={{ display: "flex", flexDirection: "column" as const, gap: 8 }}>
            {orgs.map((o: any) => (
              <Card key={o.org_id} style={{ padding: 14 }}>
                ...
                <div style={{ fontSize: 10, color: C.dim, fontFamily: "'SF Mono', monospace" }}>{o.org_id}</div>
              </Card>
            ))}
          </div>
        )}
